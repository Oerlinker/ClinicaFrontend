...
export default function AntecedentesAdmin() {
    ...
    const {data: antecedentes = [], isLoading: loadingAnte} = useQuery({
        ...
        queryFn: () => API.get<Antecedente[]>(`/api/antecedentes/usuario/${pacienteId}`).then(res => res.data),
        enabled: !!pacienteId
    });
    ...
    const createMutation = useMutation({
        mutationFn: (newAnt: { usuarioId: number; tipo: string; descripcion: string }) =>
            API.post('/api/antecedentes', newAnt),
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (upd: { id: number; usuarioId: number; tipo: string; descripcion: string }) =>
            API.put(`/api/antecedentes/${upd.id}`, {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id: number) => API.delete(`/api/antecedentes/${id}`),
        onSuccess: () => {
